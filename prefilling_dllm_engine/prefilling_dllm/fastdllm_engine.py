...
from prefilling_dllm.engine.model_runner import AutoModelRunner
try:
    from prefilling_dllm.layers.attention.ops.triton_eviction_scorer import (
...
from prefilling_dllm.utils.context import reset_context_diffusion_lm, set_context_diffusion_lm
...
class FastDLLMDreamEngine:
    """Offline Fast-DLLM decode path on top of the prefilling_dllm Dream runner.
    ...
    def __init__(
        ...
        *,
        ...
        shm_name: str = "prefilling_dllm_fastdllm",
    ) -> None:
    ...
    def _set_replace_context(self, context_len: int, block_len: int, slot_mapping: torch.Tensor) -> None:
        if block_len % self.block_length != 0:
            raise ValueError(
                f"prefilling_dllm KV loader requires active length to be a multiple of "
