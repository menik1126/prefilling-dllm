...
class FastDLLMParallelComp:
    ...
    def _score_attention_mask(
        ...
        *,
        ...
        if (mode or "").lower() == "full_query_bidirectional":
            # Only scoring query/draft rows change; preserve prefix/chunk visibility.
            mask = self._chunk_full_query_causal_mask(
                prefix_len, chunk_len, query_len, current_only=current_only,
            )
            query_start = chunk_len if current_only else prefix_len + chunk_len
            mask[:, :, query_start:, :] = 0
            return mask
            ...
            for offset in range(0, len(group), batch_size):
                ...
                with torch.inference_mode():
                    outputs = self._model_forward(
                        self._ids_batch_tensor(rows),
                        attention_mask=attention_mask,
                        use_cache=os.environ.get("SCORE_KV_EXPERIMENT") == "reuse",
                        return_dict=True,
                    )
                logits = outputs.logits
                if int(positions.max().item()) - 1 >= logits.shape[1]:
                ...
                if os.environ.get("SCORE_KV_EXPERIMENT") == "reuse":
                    if not hasattr(self, "_score_kv_parts"):
                        self._score_kv_parts = {}
                    for row, (idx, _) in enumerate(batch):
                        row_cache = [(k[row:row+1], v[row:row+1]) for k, v in outputs.past_key_values]
                        self._score_kv_parts[idx] = self._slice_cache(row_cache, 0, prefix_len + chunk_len)
                    # Retain only global top-k so memory does not scale with the document.
                    keep = set(sorted(scores, key=lambda i: (-scores[i], i))[:self.config.topk_chunks])
                    self._score_kv_parts = {i: c for i, c in self._score_kv_parts.items() if i in keep}
        return scores
    ...
    def score_chunks_draft_confidence_batched(self, prefix_ids, candidate_chunks, query_ids):
        """One full-attention, all-mask draft forward per candidate; no rescoring."""
        if self.model_backend != "dream" or self.config.temperature != 0:
            raise ValueError("draft_generation_confidence requires Dream and temperature=0")
        draft_len = int(self.config.score_draft_tokens)
        if draft_len <= 0:
            raise ValueError("draft_generation_confidence requires positive draft length")
        groups = {}
        for idx, chunk in enumerate(candidate_chunks):
            groups.setdefault(len(chunk), []).append((idx, list(chunk)))
        scores = {}
        batch_size = max(1, int(self.config.score_batch_size or 1))
        for chunk_len, group in groups.items():
            start = len(prefix_ids) + chunk_len + len(query_ids)
            for offset in range(0, len(group), batch_size):
                batch = group[offset:offset + batch_size]
                rows = [list(prefix_ids) + chunk + list(query_ids)
                        + [self._mask_token_id()] * draft_len for _, chunk in batch]
                with torch.inference_mode():
                    outputs = self._model_forward(
                        self._ids_batch_tensor(rows), attention_mask="full",
                        use_cache=False, return_dict=True,
                    )
                    # Dream predicts slot t from logits[t-1]. All slots remain masked.
                    logits = outputs.logits[:, start - 1:start + draft_len - 1, :].float()
                    token_ids = logits.argmax(dim=-1)
                    selected_logits = logits.gather(-1, token_ids.unsqueeze(-1)).squeeze(-1)
                    log_probs = selected_logits - torch.logsumexp(logits, dim=-1)
                    values = log_probs.mean(dim=-1).tolist()
                for (idx, _), value in zip(batch, values):
                    scores[idx] = value if math.isfinite(value) else float("-inf")
        return scores

    def score_shared_prefix_chunk_draft(self, prefix_ids, chunk_ids, query_ids):
        """Generate with query, then causally score draft against pristine context KV."""
        assert self.model_backend == "dream" and query_ids
        assert self.config.score_attention_mask == "full"
        context = list(prefix_ids) + list(chunk_ids)
        n = len(context)
        first_query_logit = None
        merged_cache = None
        merged_last_logit = None
        def build():
            nonlocal first_query_logit, merged_cache, merged_last_logit
            if os.environ.get("QUERY_CONDITIONED_KV") == "1":
                # No draft token or draft mask is present in this prefill.
                # Keep only prefix+chunk KV; recompute query in later stages.
                prefill_ids = context + list(query_ids)
                outputs = self._model_forward(self._ids_tensor(prefill_ids),
                    attention_mask="full", use_cache=True, return_dict=True)
                first_query_logit = outputs.logits[:, n-1:n, :].detach().clone()
                if os.environ.get("MERGE_QUERY_PREFILL") == "1":
                    merged_cache = outputs.past_key_values
                    merged_last_logit = outputs.logits[:, -1, :].detach().clone()
                cache = self._slice_cache(outputs.past_key_values, 0, n)
                assert self._cache_len(cache) == n
                return cache
            mask = self._score_attention_mask("full", q_len=n, key_len=n,
                prefix_len=len(prefix_ids), chunk_len=len(chunk_ids), query_len=0, current_only=False)
            return self._model_forward(self._ids_tensor(context), attention_mask=mask,
                use_cache=True, return_dict=True).past_key_values
        torch.cuda.synchronize()
        started = time.perf_counter()
        saved = build()
        # The generation backend may mutate supplied caches. Preserve pristine KV.
        merged = os.environ.get("MERGE_QUERY_PREFILL") == "1"
        if merged:
            assert merged_cache is not None and merged_last_logit is not None
        generation_cache = [(k.clone(), v.clone()) for k,v in (merged_cache if merged else saved)]
        draft = self.generate_from_cache(past_key_values=generation_cache,
            cache_positions=list(range(n+len(query_ids) if merged else n)),
            query_ids=[] if merged else list(query_ids),
            query_rope_start=n+len(query_ids) if merged else n,
            prefilled_last_logit=merged_last_logit if merged else None,
            max_new_tokens=int(self.config.score_draft_tokens),
            diffusion_steps=int(self.config.score_draft_steps or 1),
            force_diffusion=bool(self.config.score_draft_fixed_steps))
        del generation_cache
        if not draft or self._mask_token_id() in draft:
            raise RuntimeError("Incomplete per-chunk draft")
        mode = os.environ.get("DRAFT_CONTEXT_KV", "reuse")
        assert mode in {"reuse", "rebuild"}
        if mode == "rebuild":
            del saved
            saved = build()
        suffix = list(query_ids) + list(draft)
        qlen = len(suffix)
        rows = torch.arange(qlen, device=self.device)[:, None] + n
        cols = torch.arange(n + qlen, device=self.device)[None, :]
        mask = torch.zeros((qlen,n+qlen), device=self.device, dtype=self._mask_dtype)
        mask.masked_fill_(cols > rows, torch.finfo(self._mask_dtype).min)
        out = self._model_forward(self._ids_tensor(suffix), attention_mask=mask[None,None],
            position_ids=self._position_ids_from_list(range(n+qlen)),
            past_key_values=saved, use_cache=True, return_dict=True)
        if os.environ.get("INCLUDE_QUERY_SCORE") == "1":
            assert first_query_logit is not None, "Joint scoring requires query-conditioned prefill"
            # Dream target t uses logits[t-1]. The first query target needs
            # the last context-position logit, which is not in suffix outputs.
            logits = torch.cat([first_query_logit, out.logits[:, :qlen-1, :]], dim=1).float()
            targets = self._ids_tensor(suffix)
        else:
            logits = out.logits[:,len(query_ids)-1:qlen-1,:].float()
            targets = self._ids_tensor(draft)
        assert logits.shape[1] == targets.shape[1]
        score = (logits.gather(-1,targets.unsqueeze(-1)).squeeze(-1) - torch.logsumexp(logits,-1)).mean().item()
        assert math.isfinite(score)
        if os.environ.get("DRAFT_CONTEXT_VERIFY") == "1":
            reference = self.score_chunk_self_information(prefix_ids,chunk_ids,suffix,
                score_token_mask=[False]*len(query_ids)+[True]*len(draft))
            print(f"SHARED_KV_VERIFY cached={score:.8f} full={reference:.8f} absdiff={abs(score-reference):.8f}",flush=True)
        torch.cuda.synchronize()
        print(f"SHARED_KV_STAGE mode={mode} seconds={time.perf_counter()-started:.6f}",flush=True)
        return score
    ...
    def select_chunks(
        ...
        scores: Dict[int, float] = {}
        if self.config.score_mode == "draft_generation_confidence":
            scores.update(self.score_chunks_draft_confidence_batched(
                prefix_ids, candidate_chunks, scoring_query_ids,
            ))
        if (
        ...
        for idx, chunk_ids in enumerate(candidate_chunks):
            ...
            if self.config.score_mode in {"self_information", "draft_self_information"}:
            ...
            elif self.config.score_mode == "shared_prefix_chunk_draft":
                score = self.score_shared_prefix_chunk_draft(prefix_ids, chunk_ids, scoring_query_ids)
            elif self.config.score_mode in {"per_chunk_draft_self_information", "per_chunk_draft_only_causal"}:
                per_chunk_query_ids = list(scoring_query_ids)
                draft_ids = []
                if self.config.score_mode == "per_chunk_draft_only_causal":
                    assert self.config.score_draft_tokens > 0
                    assert self.config.score_attention_mask == "full"
                ...
                if self.config.score_mode == "per_chunk_draft_only_causal":
                    if not draft_ids:
                        raise RuntimeError("Empty per-chunk draft")
                    score = self.score_chunk_self_information(
                        prefix_ids, chunk_ids, per_chunk_query_ids,
                        score_token_mask=[False] * len(scoring_query_ids) + [True] * len(draft_ids))
                else:
    ...
    def build_prefill_cache(
        ...
        *,
        ...
        experiment = os.environ.get("SCORE_KV_EXPERIMENT", "")
        if experiment:
            assert experiment in {"reuse", "rebuild"}
            assert self.config.chunk_position_mode == "reuse"
            assert self.config.score_mode == "draft_self_information"
            assert self.config.score_attention_mask == "full"
            assert self.config.token_capacity == 0 and not self.config.keep_first_chunk
            assert self.config.topk_chunks > 0
            self._score_kv_parts = {}
            torch.cuda.synchronize()
        ...
        if prefix_ids and not experiment:
        ...
        for chunk_order, chunk_index in enumerate(selected_indices):
            ...
            if experiment:
                if experiment == "reuse":
                    joint_cache = self._score_kv_parts.pop(chunk_index)
                    if os.environ.get("SCORE_KV_VERIFY") == "1":
                        ids = list(prefix_ids) + chunk_ids
                        mask = self._score_attention_mask("full", q_len=len(ids), key_len=len(ids),
                            prefix_len=len(prefix_ids), chunk_len=len(chunk_ids), query_len=0, current_only=False)
                        reference = self._model_forward(self._ids_tensor(ids), attention_mask=mask,
                            use_cache=True, return_dict=True).past_key_values
                        diffs = [(a.float()-b.float()).abs() for pair, ref in zip(joint_cache, reference) for a,b in zip(pair,ref)]
                        assert all(torch.isfinite(x).all() for x in diffs)
                        print(f"KV_VERIFY max_abs={max(x.max().item() for x in diffs):.6f} mean_abs={sum(x.mean().item() for x in diffs)/len(diffs):.6f}", flush=True)
                        del reference, diffs
                else:
                    # Same prefix-causal / chunk-bidirectional structure as scorer;
                    # query cannot influence these KV, so it is omitted on rebuild.
                    ids = list(prefix_ids) + chunk_ids
                    mask = self._score_attention_mask("full", q_len=len(ids), key_len=len(ids),
                        prefix_len=len(prefix_ids), chunk_len=len(chunk_ids), query_len=0, current_only=False)
                    joint_cache = self._model_forward(self._ids_tensor(ids), attention_mask=mask,
                        use_cache=True, return_dict=True).past_key_values
                if chunk_order == 0 and prefix_ids:
                    cache_parts.append(self._slice_cache(joint_cache, 0, len(prefix_ids)))
                    cache_positions.extend(prefix_positions)
                    cache_len += len(prefix_ids)
                chunk_cache = self._slice_cache(joint_cache, len(prefix_ids), len(prefix_ids) + len(chunk_ids))
            else:
        ...
        if experiment:
            self._score_kv_parts = {}
            torch.cuda.synchronize()
            print(f"KV_EXPERIMENT mode={experiment} score_s={score_seconds:.6f} build_s={time.perf_counter()-build_started:.6f} candidates={len(candidate_chunks)} selected={len(selected_indices)}", flush=True)
    ...
    def generate_from_cache(
        ...
        *,
        ...
        prefilled_last_logit: Optional[torch.Tensor] = None,
    ) -> List[int]:
        mask_token_id = self._mask_token_id()
        query_ids = list(query_ids)
        cache_positions = list(cache_positions)
        ...
        active_cache = past_key_values
        active_positions = list(cache_positions)
        last_context_logit: Optional[torch.Tensor] = prefilled_last_logit
        if query_ids:
            active_cache, last_context_logit = self._prefill_plain(
                query_ids,
                query_positions,
    ...
    def generate(
        ...
        *,
        ...
        if self.config.cache_build_mode in {"full_prompt_mask", "full_prompt_query"}:
            ...
            result = self.generate_full_prompt_mask(
                prefix_ids=prefix_ids,
                context_ids=context_ids,
                query_ids=query_ids,
                scoring_query_ids=scoring_query_ids,
            ...
            torch.cuda.synchronize()
            print(f"FULL_RECOMPUTE_E2E position={self.config.chunk_position_mode} seconds={time.perf_counter()-timed_start:.6f}", flush=True)
            return result
        ...
        if os.environ.get("SCORE_KV_EXPERIMENT"):
            torch.cuda.synchronize()
            print(f"KV_E2E mode={os.environ['SCORE_KV_EXPERIMENT']} seconds={time.time()-start:.6f}", flush=True)
        self.total_generation_time += time.time() - start
        self.generated_token_num += sum(1 for token_id in generated_ids if token_id != self.tokenizer.eos_token_id)
