...
RUN_STAMP="${RUN_STAMP:-20260525_dream_blocklen_probe_mfen_numberstring_maxnew64}"
BLOCKS=(${BLOCKS:-16 32 64})
...
echo "START Dream block-length probe $(date '+%F %T')"
...
for BLOCK in "${BLOCKS[@]}"; do
  TAG="dream_pc_best47_mfen_maxnew64_block${BLOCK}_${RUN_STAMP}"
  OUT_DIR="${ROOT}/results_longbench_fastdllm_parallelcomp_${TAG}"
  LOG_FILE="${ROOT}/logs/${TAG}.log"
  echo "RUN LongBench multifieldqa_en block_length=${BLOCK} tag=${TAG} $(date '+%F %T')"
  "${PY}" eval_fastdllm_parallelcomp_longbench.py \
    ...
    > "${LOG_FILE}" 2>&1
  echo "DONE LongBench multifieldqa_en block_length=${BLOCK} $(date '+%F %T')"
done

for BLOCK in "${BLOCKS[@]}"; do
  TAG="dream_pc_best47_infinite_number_string_maxnew64_block${BLOCK}_${RUN_STAMP}"
  OUT_DIR="${ROOT}/results_infinitebench_fastdllm_parallelcomp_${TAG}"
  LOG_FILE="${ROOT}/logs/${TAG}.log"
  echo "RUN InfiniteBench number_string block_length=${BLOCK} tag=${TAG} $(date '+%F %T')"
  "${PY}" eval_fastdllm_parallelcomp_infinitebench.py \
    "${COMMON_ARGS[@]}" \
    --tasks number_string \
    --max_examples 0 \
    ...
    --block_length "${BLOCK}" \
    > "${LOG_FILE}" 2>&1
  echo "DONE InfiniteBench number_string block_length=${BLOCK} $(date '+%F %T')"
done

echo "DONE_ALL Dream block-length probe $(date '+%F %T')"
