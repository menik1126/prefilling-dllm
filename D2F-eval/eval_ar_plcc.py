...
def keep_window(obj, keep_len, strategy):
    """Keep the left/right/middle window of a sequence-like object."""
    if keep_len is None or keep_len <= 0 or len(obj) <= keep_len:
        return obj
    if strategy == 'left':
        return obj[:keep_len]
    if strategy == 'right':
        return obj[-keep_len:]
    if strategy == 'middle':
        start = max((len(obj) - keep_len) // 2, 0)
        end = start + keep_len
...
class ARModel:
    def __init__(self, pretrained, max_length, max_new_tokens, rope_scale_factor=1.0,
        ...
        print(f"  Truncate strategy    : keep_{self.truncate_strategy}")
    ...
    def generate_prompts(self, prompts):
        ...
        for prompt in prompts:
            ...
            if inputs.input_ids.shape[1] > self.prompt_limit:
                inputs['input_ids'] = keep_window(
                    inputs.input_ids, self.prompt_limit, self.truncate_strategy
                )
                inputs['attention_mask'] = keep_window(
                    inputs.attention_mask, self.prompt_limit, self.truncate_strategy
                )
...
def main():
    ...
    parser.add_argument('--truncate_strategy', default='right',
                        choices=['left', 'right', 'middle'],
                        help='Prompt truncation strategy: keep leftmost, rightmost, or middle window.')
    ...
    print(f"Truncate strategy  : keep_{args.truncate_strategy}")
    ...
    for cfg in args.configs:
        ...
        trunc_tag = f"_trunc{args.truncate_strategy}"
        out_file = os.path.join(
            args.output_dir,
            f"{args.model_tag}_plcc_{cfg}_top{int(args.top_percent)}pct{scale_tag}{trunc_tag}_predictions.jsonl"
        )
            ...
            for cat, line_nums in completion_lines.items():
                ...
                for ln in line_nums:
                    ...
                    if len(prompt) > max_prompt_chars:
                        prompt = keep_window(prompt, max_prompt_chars, args.truncate_strategy)
    ...
    combined_file = os.path.join(
        args.output_dir,
        f"{args.model_tag}_plcc_all_top{int(args.top_percent)}pct_trunc{args.truncate_strategy}_metrics.json"
    )
