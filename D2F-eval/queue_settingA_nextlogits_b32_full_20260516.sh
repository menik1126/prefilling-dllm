...
TAG=settingA_nextlogits_b32_full_20260516_1705
...
run_longbench_lane() {
  local gpu="$1"
  shift
  local out="/home/ma-user/work/Discrete-Diffusion-Forcing/D2F-eval/results_longbench_all_n0_cap128_${TAG}"
  local run_name="dream_longbench_${TAG}"
  local log="logs/${TAG}_longbench_gpu${gpu}.log"
  {
...
echo "[$(date '+%F %T')] starting LongBench ${TAG}"
...
p3=$!
wait "${p0}" "${p1}" "${p2}" "${p3}"
echo "[$(date '+%F %T')] completed LongBench ${TAG}"
